          ...
          <ul class="navbar-nav mr-auto">
             ...
             <li class="nav-item dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                   <a class="dropdown-item" href="Availability">Check Availabble Items</a>
		  ...
		  <ul class="nav navbar-nav navbar-right">
		   	<li class="nav-item dropdown  active">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                   ...
                   <a class="dropdown-item active" href="ManageItems">Items</a>
			...
			<form  method="GET" action="#">
				<div class="row">
					<div class="col text-center">
						<p class="display-4 ">Manage Items</p>
					</div>
				</div>
					...
					<div class="row">
						<div class="col col-lg-2">
							...
							<select class="form-control" id="SearchBy" name="SearchBy" required onchange="myFunction()" >
								...
								<option value="BS">Borrow Status</option>
							</select>
						</div>
		...
		<div class="row">
			<table class="table table-striped table-dark">
				...
				if ($s_by=="IID" || $s_by== "OID"){
					$kw=$_GET['query'];
					if ($s_by=="IID"){
						$q="SELECT * FROM item where item_id=$kw;";
					}
					if ($s_by=="OID"){
						$q="SELECT * FROM item where owner_id=$kw;";
					}
				}
				if ($s_by== "RM" || $s_by == "CD"){
					$kw=$_GET['query'];
					if ($s_by=="RM"){
						$q="SELECT * FROM item where remarks='$kw';";
					}
					if ($s_by=="CD"){
						$q="SELECT * FROM item where conditions='$kw';";
					}
				}
				if ($s_by== "CS"){
					$c1=$_GET['color1'];
					...
					if (!$c1==''){
						if (!$c2==''){
							if (!$size==''){
								$q="SELECT * FROM item where color1='$c1' AND color2='$c2' AND size_indicator='$size';";
							}else{
								$q="SELECT * FROM item where color1='$c1' AND color2='$c2';";
							}
						}else{
							if (!$size==''){
								$q="SELECT * FROM item where color1='$c1' AND size_indicator='$size';";
							}else{
								$q="SELECT * FROM item where color1='$c1';";
							}
						}
					} else{
						if (!$c2==''){
							if (!$size==''){
								$q="SELECT * FROM item where color2='$c2' AND size_indicator='$size';";
							}else{
								$q="SELECT * FROM item where color2='$c2';";
							}
						}else{
							$q="SELECT * FROM item where size_indicator='$size';";
						}
					}
				}
				if ($s_by== "TBM"){
					...
					if (!$c1==''){
						if (!$c2==''){
							if (!$size==''){
								$q="SELECT * FROM item where type='$c1' AND brand='$c2' AND model='$size';";
							}else{
								$q="SELECT * FROM item where type='$c1' AND brand='$c2';";
							}
						}else{
							if (!$size==''){
								$q="SELECT * FROM item where type='$c1' AND model='$size';";
							}else{
								$q="SELECT * FROM item where type='$c1';";
							}
						}
					} else{
						if (!$c2==''){
							if (!$size==''){
								$q="SELECT * FROM item where brand='$c2' AND model='$size';";
							}else{
								$q="SELECT * FROM item where brand='$c2';";
							}
						}else{
							$q="SELECT * FROM item where model='$size';";
						}
					}
				}
				if ($s_by== "BS"){
					...
					if ($kw=="SRN"){
						$q="SELECT * FROM item where borrow_status='BORROWED';";
					}
					if ($kw=="ETC"){
						$q="SELECT * FROM item where not borrow_status='BORROWED' and not borrow_status='IN STOCK';";
					}
				}
				...
				if ($result->num_rows>0){
					while($row=$result->fetch_array()){
						...
						echo '<td><form  method="post" action="ItemAction" target="_blank"><input type="hidden" id="act" name="act" value="edit"><input type="hidden" id="ItemID" name="ItemID" value="'.$row["item_id"].'"><button type="submit" class="btn btn-primary btn-sm">Edit</button></form></td>';
						echo '<td><form  method="post" action="ItemAction" target="_blank"><input type="hidden" id="act" name="act" value="delete"><input type="hidden" id="ItemID" name="ItemID" value="'.$row["item_id"].'"><button type="submit" class="btn btn-danger btn-sm">Delete</button></form></td>';
					}
				}	
				?>
				</tbody>
