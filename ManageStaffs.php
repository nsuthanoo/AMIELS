	...
	if (x == "CID"||x=="CLI"){
		...
		if(x == "CNN"){
			...
			if (x == "CFN"){
				...
				if (x == "CY"){
					document.getElementById("fields").innerHTML ='<div class="row"><div class="col"><label for="year" style="font-size: 30px">Year</label><input type="text"  class="form-control" id="year" name="year"></div><div class="col-md-auto"><button type="submit" class="btn btn-success btn-lg" style="margin-top: 30px">Submit</button></div></div>';
				} else{
					if (x == "CPN") {
		  ...
		  <ul class="nav navbar-nav navbar-right">
		   	<li class="nav-item dropdown  active">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                   ...
                   <a class="dropdown-item active" href="ManageStaffs">Staffs</a>
			...
			<form  method="GET" action="#">
				<div class="row">
					<div class="col text-center">
						<p class="display-4 ">Manage Staffs</p>
					</div>
				</div>
		...
		<div class="row">
			<table class="table table-striped table-dark">
				...
				if ($s_by== "CID"||$s_by=="CLI"){
					$kw=$_GET['query'];
					if ($s_by=="CID"){
						$q="SELECT *FROM staff WHERE staff_id= '$kw';";
					}
					if ($s_by=="CLI"){
						$q="SELECT *FROM staff WHERE line_id= '$kw';";
					}
				}
				if ($s_by == "CNN"){
					$kw=$_GET['query'];
					if ($s_by=="CNN"){
						$q="SELECT *FROM staff WHERE nickname= '$kw';";
					}
				}
				if ($s_by== "CFN"){
					$fn=$_GET['FName'];
					...
					if ($s_by=="CFN"){
						...
						$q="SELECT *FROM staff WHERE $fsn;";
					}
				}
				if ($s_by== "CY"){
					$yy=$_GET['year'];
					$q="SELECT *FROM staff WHERE year =$yy;";
				}
				if ($s_by== "CPN"){
					$kw=$_GET['tel'];
					if ($s_by=="CPN"){
						$q="SELECT *FROM staff WHERE tel_no= '$kw';";
					}
				}
				...
				if ($result->num_rows>0){
					while($row=$result->fetch_array()){
						echo "<tr>";
						echo "<th scope='row'>".$row["staff_id"]."</th>";
						echo "<td>".$row["first_name"]." ".$row["surname"]."</td>";
						echo "<td>".$row["nickname"]."</td>";
						echo "<td>".$row["year"]."</td>";
						echo "<td>".$row["tel_no"]."</td>";
						echo "<td>".$row["line_id"]."</td>";
						echo '<td><form  method="post" action="StaffAction" target="_blank"><input type="hidden" id="act" name="act" value="edit"><input type="hidden" id="StaffID" name="StaffID" value="'.$row["staff_id"].'"><button type="submit" class="btn btn-primary btn-sm">Edit</button></form></td>';
						echo '<td><form  method="post" action="StaffAction" target="_blank"><input type="hidden" id="act" name="act" value="delete"><input type="hidden" id="StaffID" name="StaffID" value="'.$row["staff_id"].'"><button type="submit" class="btn btn-danger btn-sm">Delete</button></form></td>';
					}
				}	
				?>
				</tbody>
