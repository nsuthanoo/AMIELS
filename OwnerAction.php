...
<html lang="en">
  ...
  <body style="background-image: url(images/ins.jpg); background-size: cover;">
	  <div class="container align-content-center" style="color: black; background-color: whitesmoke; padding-left: 7%; padding-right: 7%;margin-top: 5%; margin-bottom: 5%">
		  ...
		  $sid=$_POST['OwnerID'];
		  if ($_POST['act']=='edit'){
			  $q= 'SELECT * FROM owner WHERE owner_id='.$sid.';';
			  echo '<p class="display-4">Edit Owner ID: '.$sid.'</p>';
			  ...
			  echo '<form  method="post"  action="#">
				<div class="row">
						<div class="col">
							<label for="StudentID" style="font-size: 30px">Owner ID</label>
						...
						<div class="col">
							...
							<input type="text" class="form-control" id="Nickname" name="Nickname" maxlength="100" value="'.$nickname.'">
						</div>
					</div>
					<br>
					<div class="row">
						<div class="col">
							...
							<input type="text" class="form-control" id="Firstname" name="Firstname" maxlength="100" value="'.$first_name.'">
					...
					<div class="row">
						<div class="col">
							...
							<input type="text" class="form-control" id="Surname" name="Surname" maxlength="100" value="'.$surname.'">
					...
					<br>
					<input type="hidden" id="OwnerID" name="OwnerID" value="'.$sid.'">
		  ...
		  if ($_POST['act']=='delete'){
			  echo '<div class="container-fluid text-center"><p class="display-3">Delete Owner ID: '.$sid.'</p>';
			  ...
			  <input type="hidden" id="OwnerID" name="OwnerID" value="'.$sid.'"><input type="hidden" id="act" name="act" value="do_remove"><button type="submit" class="btn btn-danger btn-lg">Delete</button>&nbsp;&nbsp;<a href="#" class="btn btn-outline-danger btn-lg" onclick="window.close();return false;">Cancel</a></form></div>';
		  }
		   if ($_POST['act']=='do_edit'){
			   ...
			   $email = $_POST['email'];
			   $q="UPDATE owner SET first_name='$first_name', surname='$surname', nickname='$nickname', year='$year', faculty='$faculty', tel_no='$tel_no', line_id='$line_id', email='$email' WHERE owner_id='$sid'";
			   $result=mysqli_query($con, $q);
			   if (!$result) {
					trigger_error('Invalid query: ' . $con->error);
			   } else{
				   echo '<div class="container-fluid text-center"><p class="display-3">Owner ID: '.$sid.' Updated Successfully</p><a href="#" class="btn btn-outline-danger btn-lg" onclick="window.close();return false;">Close</a>';
			   }
				  
		   }
		   if ($_POST['act']=='do_remove'){
			   $q="DELETE FROM owner WHERE owner_id='$sid';";
			   $result=mysqli_query($con, $q);
			   if (!$result) {
				   echo '<div class="container-fluid text-center"><p class="display-3">An error occured. OwnerID might be in some items. Please remove those items before removing the owner.</p><a href="#" class="btn btn-outline-danger btn-lg" onclick="window.close();return false;">Close</a>';
			   }else{
					echo '<div class="container-fluid text-center"><p class="display-3">Owner ID: '.$sid.' Removed Successfully</p><a href="#" class="btn btn-outline-danger btn-lg" onclick="window.close();return false;">Close</a>';
