		  ...
		  <ul class="nav navbar-nav navbar-right">
		   	<li class="nav-item dropdown  active">
                ...
                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                   ...
                   <a class="dropdown-item active" href="ManageOwners">Owners</a>
                </div>
             </li>
			...
			<form  method="GET" action="#">
				<div class="row">
					<div class="col text-center">
						<p class="display-4 ">Manage Owners</p>
					</div>
				</div>
					...
					<div class="row">
						<div class="col col-lg-2">
							...
							<select class="form-control" id="SearchBy" name="SearchBy" required onchange="myFunction()" >
								...
								<option value="CID">Owner ID</option>
		...
		<div class="row">
			<table class="table table-striped table-dark">
				...
				if ($s_by== "CID"||$s_by=="CLI"){
					$kw=$_GET['query'];
					if ($s_by=="CID"){
						$q="SELECT *FROM owner WHERE owner_id= '$kw';";
					}
					if ($s_by=="CLI"){
						$q="SELECT *FROM owner WHERE line_id= '$kw';";
					}
				}
				if ($s_by == "CNN"){
					$kw=$_GET['query'];
					if ($s_by=="CNN"){
						$q="SELECT *FROM owner WHERE nickname= '$kw';";
					}
				}
				if ($s_by== "CFN"){
					$fn=$_GET['FName'];
				...
				if ($s_by== "CYF"){
					...
					$q="SELECT *FROM owner WHERE $yf;";
				}
				if ($s_by== "CPN"){
					$kw=$_GET['tel'];
					if ($s_by=="CPN"){
						$q="SELECT *FROM owner WHERE tel_no= '$kw';";
					}
				}
				if ($s_by== "CEM"){
					$kw=$_GET['email'];
					$q="SELECT *FROM owner WHERE email='$kw';";
				}
				...
				if ($result->num_rows>0){
					while($row=$result->fetch_array()){
						echo "<tr>";
						echo "<th scope='row'>".$row["owner_id"]."</th>";
						...
						echo '<td><form  method="post" action="OwnerAction" target="_blank"><input type="hidden" id="act" name="act" value="edit"><input type="hidden" id="OwnerID" name="OwnerID" value="'.$row["owner_id"].'"><button type="submit" class="btn btn-primary btn-sm">Edit</button></form></td>';
						echo '<td><form  method="post" action="OwnerAction" target="_blank"><input type="hidden" id="act" name="act" value="delete"><input type="hidden" id="OwnerID" name="OwnerID" value="'.$row["owner_id"].'"><button type="submit" class="btn btn-danger btn-sm">Delete</button></form></td>';
					}
				}	
				?>
				</tbody>
